 ...
 *
 * MixedPoissonThermal intended to show how to solve mixed formulation of the Dirichlet
...
struct MixedPoissonThermal {

  MixedPoissonThermal(MoFEM::Interface &m_field) : mField(m_field) {}
...
//! [Run programme]
MoFEMErrorCode MixedPoissonThermal::runProblem() {
  MoFEMFunctionBegin;
  CHKERR readMesh();
  CHKERR setupProblem();
...
//! [Read mesh]
MoFEMErrorCode MixedPoissonThermal::readMesh() {
  MoFEMFunctionBegin;
...
//! [Set up problem]
MoFEMErrorCode MixedPoissonThermal::setupProblem() {
  MoFEMFunctionBegin;
...
MoFEMErrorCode MixedPoissonThermal::setIntegrationRules() {
  MoFEMFunctionBegin;
...
//! [Create common data]
MoFEMErrorCode MixedPoissonThermal::createCommonData() {
  MoFEMFunctionBegin;
...
//! [Assemble system]
MoFEMErrorCode MixedPoissonThermal::assembleSystem() {
  MoFEMFunctionBegin;
  PipelineManager *pipeline_mng = mField.getInterface<PipelineManager>();
...
//! [Solve]
MoFEMErrorCode MixedPoissonThermal::solveSystem() {
  MoFEMFunctionBegin;
  PipelineManager *pipeline_mng = mField.getInterface<PipelineManager>();
...
//! [Refine]
MoFEMErrorCode MixedPoissonThermal::refineOrder() {
  MoFEMFunctionBegin;
...
MoFEMErrorCode MixedPoissonThermal::solveRefineLoop() {
  MoFEMFunctionBegin;
...
//! [Check error]
MoFEMErrorCode MixedPoissonThermal::checkError(int iter_num) {
  MoFEMFunctionBegin;
  PipelineManager *pipeline_mng = mField.getInterface<PipelineManager>();
  pipeline_mng->getDomainLhsFE().reset();
  pipeline_mng->getDomainRhsFE().reset();
...
//! [Output results]
MoFEMErrorCode MixedPoissonThermal::outputResults(int iter_num) {
  MoFEMFunctionBegin;
  PipelineManager *pipeline_mng = mField.getInterface<PipelineManager>();
  pipeline_mng->getDomainLhsFE().reset();
  auto post_proc_fe =
...
//! [OpError]
MoFEMErrorCode MixedPoissonThermal::OpError::doWork(int side, EntityType type,
                                             EntData &data) {
  MoFEMFunctionBegin;
  ...
  for (int gg = 0; gg != nb_integration_pts; ++gg) {
    ...
    double diff = t_val - MixedPoissonThermal::analyticalFunction(
                              t_coords(0), t_coords(1), t_coords(2));
    error_l2 += alpha * sqr(diff);

    VectorDouble vec = MixedPoissonThermal::analyticalFunctionGrad(
        t_coords(0), t_coords(1), t_coords(2));
  ...
  CHKERR MixedPoissonThermal::getTagHandle(mField, "ERROR_L2_NORM", MB_TYPE_DOUBLE,
                                    th_error_l2);
  CHKERR MixedPoissonThermal::getTagHandle(mField, "ERROR_H1_SEMINORM", MB_TYPE_DOUBLE,
                                    th_error_h1);
  CHKERR MixedPoissonThermal::getTagHandle(mField, "ERROR_INDICATOR", MB_TYPE_DOUBLE,
                                    th_error_ind);
...
int main(int argc, char *argv[]) {
  ...
  MOFEM_LOG_TAG("EXAMPLE", "MixedPoissonThermal");
  ...
  try {
    ...
    //! [MixedPoissonThermal]
    MixedPoissonThermal ex(m_field);
