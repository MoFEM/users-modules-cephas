...
MoFEMErrorCode Example::OpRhs::doWork(int side, EntityType type,
  ...
  if (nb_dofs) {
    ...
    for (size_t gg = 0; gg != nb_gauss_pts; ++gg) {
      ...
      const double dot = t_q(i) * t_q(i) + t_dot_q(i) * t_q(i);
      const double res = dot - 1;

      for (size_t bb = 0; bb != nb_dofs; ++bb) {
        nf[bb] -= alpha * t_base * (res + reg * t_div + t_dot_div);
        ++t_base;
      }
        ...
        for (size_t cc = 0; cc != col_nb_dofs; ++cc) {
          loc_mat(rr, cc) -=
              ...
              (
                  ...
                  (t_q(i) * ts_a * t_col_base(i) + t_dot_q(i) * t_col_base(i)) +
