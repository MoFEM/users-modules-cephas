...
OpCalculatePlasticFlowLhs_dEP_ALE::OpCalculatePlasticFlowLhs_dEP_ALE(
    const std::string row_field_name, const std::string col_field_name,
    boost::shared_ptr<CommonData> common_data_ptr,
    boost::shared_ptr<MatrixDouble> m_D_ptr,
...
MoFEMErrorCode OpCalculatePlasticFlowLhs_dEP_ALE::iNtegrate(
  ...
  auto t_diff_plastic_strain = diff_tensor();
  constexpr auto size_symm = (SPACE_DIM * (SPACE_DIM + 1)) / 2;
  auto t_omega = getFTensor1FromMat<SPACE_DIM>(*velocityPtr);
  ...
  for (size_t gg = 0; gg != nb_integration_pts; ++gg) {
    ...
    for (; rr != AssemblyDomainEleOp::nbRows / size_symm; ++rr) {
      ...
      for (size_t cc = 0; cc != AssemblyDomainEleOp::nbCols / size_symm; ++cc) {
        ...
        t_mat(i, j, k, l) += t_row_base * (t_D(i, j, m, n) *
                                           (alpha  *
...
MoFEMErrorCode OpCalculatePlasticFlowLhs_dTAU_ALE::iNtegrate(
  ...
  for (size_t gg = 0; gg != nb_integration_pts; ++gg) {
    ...
    for (size_t rr = 0; rr != AssemblyDomainEleOp::nbRows / size_symm; ++rr) {
      ...
      auto t_col_diff_base = col_data.getFTensor1DiffN<SPACE_DIM>(gg, 0);
      for (size_t cc = 0; cc != AssemblyDomainEleOp::nbCols; cc++) {
        t_mat(i, j) -= alpha  * t_row_base * t_flow_stress(i, j) *
  ...
  for (size_t gg = 0; gg != nb_integration_pts; ++gg) {
    ...
    for (; rr != AssemblyDomainEleOp::nbRows; ++rr) {
      ...
      for (size_t cc = 0; cc != AssemblyDomainEleOp::nbCols; ++cc) {
        ...
        const double c0_p = c0 * ((t_col_diff_base(i) * t_omega(i)));
        *mat_ptr += c0_p * t_row_base;
