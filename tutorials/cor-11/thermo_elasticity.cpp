    ...
    for (int gg = 0; gg != nbIntegrationPts; ++gg) {
      ...
      for (int rr = 0; rr != nbRows / 3; ++rr) {
        ...
        for (int cc = 0; cc != nbCols / 3;++cc) {
          ...
          // t_m(i, k) += 
          //     a * (tDT(i, j, k, l) * (t_row_diff_base(j) * t_col_diff_base(l)));    
          t_m(i, k) += 
              a * (tDT(i, j, k, l) * analyticalFunction(t_coords(0), t_coords(1), t_coords(2)) * (t_row_diff_base(j) * t_col_diff_base(l)));
...
struct OpPressure : MoFEM::FaceElementForcesAndSourcesCore::UserDataOperator {
  ...
  OpPressure(const double pressure_val = 1.0)
