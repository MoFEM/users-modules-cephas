...
MoFEMErrorCode ThermalElement::OpHeatCapacityLhs::doWork(
  ...
  try {
    ...
    for (unsigned int gg = 0; gg < row_data.getN().size1(); gg++) {
      ...
      // ublas
      noalias(M) += val * outer_prod(row_data.getN(gg, nb_row),
...
ThermalElement::OpHeatFlux::doWork(int side, EntityType type,
  ...
  for (unsigned int gg = 0; gg < data.getN().size1(); gg++) {
    ...
    double flux;
    if (hoGeometry) {
      double area = norm_2(getNormalsAtGaussPts(gg)) *
                    0.5; // cblas_dnrm2(3,&getNormalsAtGaussPts()(gg,0),1);
      flux = dAta.dAta.data.value1 *
...
MoFEMErrorCode ThermalElement::OpRadiationLhs::doWork(
  ...
  try {
    ...
    for (unsigned int gg = 0; gg < row_data.getN().size1(); gg++) {
      ...
      double val =
          fOur * getGaussPts()(2, gg) * radiationConst * T3_at_Gauss_pt;
      noalias(N) += val * outer_prod(row_data.getN(gg, nb_row),
                                     col_data.getN(gg, nb_col));
    }
...
MoFEMErrorCode ThermalElement::TimeSeriesMonitor::postProcess() {
  ...
  ierr = mField.getInterface(recorder_ptr);
  CHKERRG(ierr);
  ierr = recorder_ptr->record_begin(seriesName);
  CHKERRG(ierr);
  ierr =
      recorder_ptr->record_field(seriesName, tempName, proble_bit_level, mask);
  CHKERRG(ierr);
  ierr = recorder_ptr->record_end(seriesName, ts_t);
  CHKERRG(ierr);
...
ThermalElement::addThermalElements(const std::string field_name,
  ...
  ierr = mField.add_finite_element("THERMAL_FE", MF_ZERO);
  CHKERRG(ierr);

  ierr = mField.modify_finite_element_add_field_row("THERMAL_FE", field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_col("THERMAL_FE", field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_data("THERMAL_FE", field_name);
  CHKERRG(ierr);
  if (mField.check_field(mesh_nodals_positions)) {
    ierr = mField.modify_finite_element_add_field_data("THERMAL_FE",
...
ThermalElement::addThermalFluxElement(const std::string field_name,
  ...
  ierr = mField.add_finite_element("THERMAL_FLUX_FE", MF_ZERO);
  CHKERRG(ierr);
  ierr =
      mField.modify_finite_element_add_field_row("THERMAL_FLUX_FE", field_name);
  CHKERRG(ierr);
  ierr =
      mField.modify_finite_element_add_field_col("THERMAL_FLUX_FE", field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_data("THERMAL_FLUX_FE",
                                                     field_name);
  CHKERRG(ierr);
  if (mField.check_field(mesh_nodals_positions)) {
    ierr = mField.modify_finite_element_add_field_data("THERMAL_FLUX_FE",
  ...
  for (_IT_CUBITMESHSETS_BY_BCDATA_TYPE_FOR_LOOP_(mField, SIDESET | HEATFLUXSET,
                                                  it)) {
    ierr = it->getBcDataStructure(setOfFluxes[it->getMeshsetId()].dAta);
    CHKERRG(ierr);
    rval = mField.get_moab().get_entities_by_type(
  ...
  // this is alternative method for setting boundary conditions, to bypass bu in
  // cubit file reader. not elegant, but good enough
...
MoFEMErrorCode ThermalElement::addThermalConvectionElement(
  ...
  ierr = mField.add_finite_element("THERMAL_CONVECTION_FE", MF_ZERO);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_row("THERMAL_CONVECTION_FE",
                                                    field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_col("THERMAL_CONVECTION_FE",
                                                    field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_data("THERMAL_CONVECTION_FE",
                                                     field_name);
  CHKERRG(ierr);
  if (mField.check_field(mesh_nodals_positions)) {
    ierr = mField.modify_finite_element_add_field_data("THERMAL_CONVECTION_FE",
...
MoFEMErrorCode ThermalElement::addThermalRadiationElement(
  ...
  ierr = mField.add_finite_element("THERMAL_RADIATION_FE", MF_ZERO);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_row("THERMAL_RADIATION_FE",
                                                    field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_col("THERMAL_RADIATION_FE",
                                                    field_name);
  CHKERRG(ierr);
  ierr = mField.modify_finite_element_add_field_data("THERMAL_RADIATION_FE",
                                                     field_name);
  CHKERRG(ierr);
  if (mField.check_field(mesh_nodals_positions)) {
    ierr = mField.modify_finite_element_add_field_data("THERMAL_RADIATION_FE",
...
MoFEMErrorCode ThermalElement::setTimeSteppingProblem(
  ...
  ierr = setTimeSteppingProblem(field_name, rate_name, mesh_nodals_positions);
  CHKERRG(ierr);
