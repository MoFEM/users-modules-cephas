  ...
  if (nb_dofs) {
    ...
    for (size_t gg = 0; gg != nb_gauss_pts; ++gg) {
      ...
      for (; bb != nb_dofs / 3; ++bb) {
        ...
        t_nf(i) += 2. * alpha * (t_base(j) * t_grad(i, j));
        t_nf(i) += 2. * alpha * t_div_base * (t_x(i) /*- t_X(i)*/);
        ++t_nf;
        ++t_base;
        ++t_diff_base;
      }
...
MoFEMErrorCode SimpleContactProblem::OpInternalDomainContactRhs::doWork(
  ...
  if (nb_dofs) {
    ...
    for (size_t gg = 0; gg != nb_gauss_pts; ++gg) {
      ...
      size_t bb = 0;
      for (; bb != nb_dofs / 3; ++bb) {
        t_nf(i) += 2. * alpha * t_base * t_div_stress(i);
        t_nf(i) += 2. * alpha * t_diff_base(j) * t_lambdas(i, j);
        ++t_nf;
        ++t_base;
        ++t_diff_base;
      }
  ...
  if (nb_dofs) {
    ...
    for (size_t gg = 0; gg != nb_gauss_pts; ++gg) {
      ...
      for (; bb != nb_dofs / 3; ++bb) {
        ...
        t_traction(j) += 2 * (t_base(i) * t_normal(i)) * t_field_data(j);
    ...
    for (size_t gg = 0; gg != nb_gauss_pts; ++gg) {
      ...
      for (; rr != row_nb_dofs / 3; ++rr) {
        ...
        for (size_t cc = 0; cc != col_nb_dofs / 3; ++cc) {
          t_mat_diag(i) += 2. * alpha * t_row_base(j) * t_col_diff_base(j);
          t_mat_diag(i) += 2. * alpha * t_row_div_base * t_col_base;
          ++t_col_base;
          ++t_col_diff_base;
          ++t_mat_diag;
          ++t_mat;
...
MoFEMErrorCode SimpleContactProblem::setContactOperatorsLhsOperatorsHdiv3DSurface(
  ...
  MoFEMFunctionBegin;
    ...
    //           common_data_simple_contact, cnValue));
      ...
      fe_lhs_simple_contact->getOpPtrVector().push_back(
          new OpConstrainBoundaryLhs_dU_Slave(lagrange_field_name,
          field_name,
                                              common_data_simple_contact,
                                              cnValue));

      fe_lhs_simple_contact->getOpPtrVector().push_back(
          new OpConstrainBoundaryLhs_dU_Master(lagrange_field_name,
          field_name,
                                               common_data_simple_contact,
                                               cnValue));
    ...
    // fe_lhs_simple_contact->getOpPtrVector().push_back(
    //     new OpConstrainBoundaryLhs_dU_Slave_tied(
