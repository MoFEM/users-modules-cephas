...
MoFEMErrorCode NavierStokesElement::setOperators(
  ...
  for (auto &sit : common_data->setOfBlocksData) {
    ...
    feLhs->getOpPtrVector().push_back(new OpAssembleK(common_data, sit.second));
    feLhs->getOpPtrVector().push_back(new OpAssembleG(common_data, sit.second));
...
MoFEMErrorCode NavierStokesElement::OpAssembleG::doWork(
...
MoFEMErrorCode NavierStokesElement::OpAssembleK::doWork(
    int row_side, int col_side, EntityType row_type, EntityType col_type,
    EntData &row_data, EntData &col_data) {
  MoFEMFunctionBegin;
 ...
 */
MoFEMErrorCode NavierStokesElement::OpAssembleRhsPressure::iNtegrate(EntData &data) {
  MoFEMFunctionBegin;
