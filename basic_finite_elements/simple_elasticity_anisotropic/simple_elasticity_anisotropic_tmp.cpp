...
struct OpPressure : MoFEM::FaceElementForcesAndSourcesCore::UserDataOperator {
  ...
  OpPressure(const double pressure_val = 10)
...
struct ApplyDirichletBc : public MoFEM::FEMethod {
  ...
  ApplyDirichletBc(const Range &fix_faces, const Range &fix_nodes,
                   const Range &fix_second_faces)
      : MoFEM::FEMethod(), fixFaces(fix_faces), fixNodes(fix_nodes),
        fixSecondFaces(fix_second_faces) {
    // constructor
  }
  ...
  MoFEMErrorCode postProcess() {
    ...
    for (_IT_NUMEREDDOF_ROW_FOR_LOOP_(problemPtr, dit)) {
      ...
      if (fixSecondFaces.find(dit->get()->getEnt()) != fixSecondFaces.end()) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    Range fix_faces, pressure_faces, fix_nodes, fix_second_faces;
      ...
      if (id == FIX_BRICK_FACES) { // brick-faces
        ...
        CHKERR m_field.get_moab().get_entities_by_dimension(
            meshset, 2, pressure_faces, true);
                 ...
                 FIX_FACES_Y_DIR) { // restrained faces in y direction

        CHKERR m_field.get_moab().get_entities_by_dimension(meshset, 2,
                                                            fix_second_faces, true);
        
        Range adj_ents2;
        CHKERR m_field.get_moab().get_adjacencies(fix_second_faces, 0, false, adj_ents2,
                                                  moab::Interface::UNION);

        CHKERR m_field.get_moab().get_adjacencies(fix_second_faces, 1, false, adj_ents2,
                                                  moab::Interface::UNION);
        fix_second_faces.merge(adj_ents2);
    ...
    boost::shared_ptr<FEMethod> fix_dofs_fe(
        new ApplyDirichletBc(fix_faces, fix_nodes, fix_second_faces));
