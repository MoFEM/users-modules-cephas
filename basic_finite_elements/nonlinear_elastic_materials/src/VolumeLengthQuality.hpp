...
struct VolumeLengthQuality: public NonlinearElasticElement::FunctionsToCalculatePiolaKirchhoffI<TYPE> {
    ...
    VolumeLengthQuality(VolumeLengthQualityType type, double alpha,
                        double gamma)
    ...
    MoFEMErrorCode getMaterialOptions() {
      ...
      ierr = PetscOptionsScalar("-volume_length_alpha",
                                "volume length alpha parameter", "", aLpha,
                                &aLpha, PETSC_NULL);
      CHKERRG(ierr);
      ierr = PetscOptionsScalar("-volume_length_gamma",
    ...
    MoFEMErrorCode calculateLrms() {
      ...
      for (int ee = 0; ee < 6; ee++) {
        for (int dd = 0; dd < 3; dd++) {
          deltaChi[dd] =
              coordsEdges[6 * ee + dd] - coordsEdges[6 * ee + 3 + dd];
        }
    ...
    */
    MoFEMErrorCode calculateQ() {
     ...
     virtual MoFEMErrorCode calculateP_PiolaKirchhoffI(
       ...
       ierr = getEdgesFromElemCoords();
       CHKERRG(ierr);

       ierr = this->dEterminant(this->F, detF);
       CHKERRG(ierr);
       if (this->invF.size1() != 3) {
         this->invF.resize(3, 3);
       }
       ierr = this->iNvert(detF, this->F, this->invF);
       CHKERRG(ierr);
