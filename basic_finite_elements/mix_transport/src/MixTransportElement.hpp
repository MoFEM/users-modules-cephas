  ...
  */
  ...
  struct OpDivTauU_HdivL2
    ...
    MoFEMErrorCode doWork(int side, EntityType type,
      ...
      Nf.resize(nb_row);
      Nf.clear();
      divVec.resize(data.getVectorN().size2() / 3, 0);
  ...
  struct OpRhsBcOnValues
    ...
    MoFEMErrorCode doWork(int side, EntityType type,
      ...
      for (int gg = 0; gg < nb_gauss_pts; gg++) {
        ...
        if (getNormalsAtGaussPts().size1() == (unsigned int)nb_gauss_pts) {
          noalias(nF) +=
              w * prod(data.getVectorN(gg), getNormalsAtGaussPts(gg)) * value;
        } else {
  ...
  struct OpEvaluateBcOnFluxes
    ...
    MoFEMErrorCode doWork(int side, EntityType type,
      ...
      int nb_gauss_pts = data.getVectorN().size1();
      if (3 * nb_dofs != static_cast<int>(data.getVectorN().size2())) {
      ...
      for (int gg = 0; gg < nb_gauss_pts; gg++) {
        ...
        for (int ll = 0; ll != nb_dofs; ll++) {
          ...
          FTensor::Tensor1<const double *, 3> t_n_hdiv_col(
              &data.getVectorN(gg)(0, HVEC0), &data.getVectorN(gg)(0, HVEC1),
              &data.getVectorN(gg)(0, HVEC2), 3);
  ...
  struct OpFluxDivergenceAtGaussPts
    ...
    MoFEMErrorCode doWork(int side, EntityType type,
      ...
      for (int gg = 0; gg < nb_gauss_pts; gg++) {
        ...
        flux_at_gauss_pt += prod(trans(data.getVectorN(gg)), data.getFieldData());
      }
      MoFEMFunctionReturn(0);
    }
  };
