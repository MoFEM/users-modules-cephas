...
const double u0 = 1.0; ///< inital vale on blocksets
...
struct OpAssembleSlowRhs : OpEle {
  ...
  MoFEMErrorCode doWork(int side, EntityType type, EntData &data) {
    MoFEMFunctionBegin;
    if(field == "u"){r = ru;  a1 = au1; a2 = au2; a3 = au3;}
    if(field == "v"){r = rv;  a1 = av2; a2 = av1; a3 = av3;}    
    if(field == "w"){r = rw;  a1 = aw3; a2 = aw1; a3 = aw2;}
    ...
    if (nb_dofs) {
      ...
      for (int gg = 0; gg != nb_integration_pts; ++gg) {
        ...
        const double f = a * r * t_val_u * (1.0 -  a1 * t_val_u - a2 * t_val_v - a3 * t_val_w);
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    CHKERR simple_interface->addDomainField("w", H1, AINSWORTH_LEGENDRE_BASE, 1);
    ...
    auto dot_val_ptr_u = boost::shared_ptr<VectorDouble>(data, &data->dot_val_u);
    ...
    auto dot_val_ptr_v = boost::shared_ptr<VectorDouble>(data, &data->dot_val_v);
    ...
    auto dot_val_ptr_w = boost::shared_ptr<VectorDouble>(data, &data->dot_val_w);
    ...
    if (m_field.getInterface<MeshsetsManager>()->checkMeshset(block_id1, BLOCKSET)) {
      Range inner_surface;
      ...
      if (!inner_surface.empty()) {
        ...
        CHKERR m_field.getInterface<FieldBlas>()->setField(
            u0, MBVERTEX, inner_surface_verts, "v");
      }
    }
    //blockset 2
      const int block_id2 = 2;
      if (m_field.getInterface<MeshsetsManager>()->checkMeshset(block_id2, BLOCKSET)) {
      Range inner_surface;
      CHKERR m_field.getInterface<MeshsetsManager>()->getEntitiesByDimension(
      ...
      if (m_field.getInterface<MeshsetsManager>()->checkMeshset(block_id3, BLOCKSET)) {
      Range inner_surface;
      CHKERR m_field.getInterface<MeshsetsManager>()->getEntitiesByDimension(
      ...
      if (!inner_surface.empty()) {
        ...
        CHKERR moab.get_connectivity(inner_surface, inner_surface_verts, false);
        CHKERR m_field.getInterface<FieldBlas>()->setField(
            u0, MBVERTEX, inner_surface_verts, "w");
      }
    }

    //blockset 2 
    const int block_id4 = 4;
      if (m_field.getInterface<MeshsetsManager>()->checkMeshset(block_id4, BLOCKSET)) {
      Range inner_surface;
      CHKERR m_field.getInterface<MeshsetsManager>()->getEntitiesByDimension(
          block_id4, BLOCKSET, 2, inner_surface, true);
      if (!inner_surface.empty()) {
        Range inner_surface_verts;
        CHKERR moab.get_connectivity(inner_surface, inner_surface_verts, false);
        CHKERR m_field.getInterface<FieldBlas>()->setField(
    ...
    // CHKERR m_field.getInterface<ProblemsManager>()->removeDofsOnEntities(
    //     simple_interface->getProblemName(), "v",
    //     unite(edges_verts, edges_part));
    // CHKERR m_field.getInterface<ProblemsManager>()->removeDofsOnEntities(
    //     simple_interface->getProblemName(), "w",
    //     unite(edges_verts, edges_part));
    ...
    vol_mass_ele->getOpPtrVector().push_back(new OpAssembleMass("v", "v", local_M));    
    vol_mass_ele->getOpPtrVector().push_back(new OpAssembleMass("u", "u", local_M));
    vol_mass_ele->getOpPtrVector().push_back(new OpAssembleMass("w", "w", local_M));
...
// the one species#include <BasicFiniteElements.hpp>
// using namespace MoFEM;
// static char help[] = "...\n\n";
// namespace ReactionDiffusionEquation {
// using Ele = FaceElementForcesAndSourcesCore;
