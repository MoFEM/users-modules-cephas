...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    CHKERR PetscOptionsBool("-my_is_l2",
                            "set if mesh is partitioned (this result that each "
                            "process keeps only part of the mes",
                            "", PETSC_FALSE, &is_l2, PETSC_NULL);
...
//    contact_problem->commonDataSimpleContact->forcesOnlyOnEntitiesCol = nodes;
    ...
    //   CHKERR m_field.modify_finite_element_add_field_row("L2MATERIAL",
    //                                                      "LAGMULT");
    //   CHKERR m_field.modify_finite_element_add_field_col("L2MATERIAL",
    //                                                      "LAGMULT");
    //   CHKERR m_field.modify_finite_element_add_field_data("L2MATERIAL",
    //                                                       "LAGMULT");
...
if (is_lag) {
  ...
  for (Range::iterator it_vertices = slave_tris.begin();
    ...
    if (is_l2) {
      ...
      //             "SPATIAL_POSITION", "LAGMULT", "L2MATERIAL", A);
